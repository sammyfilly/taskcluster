...
func TestLoopbackAudio(t *testing.T) {
	...
	payload := GenericWorkerPayload{
		Command: [][]string{
			...
			{"ls", "-l", devicePaths[3]},
			{"ls", "-l", devicePaths[4]},
			{"/usr/bin/env", "bash", "-c", `echo "Device: $TASKCLUSTER_AUDIO_DEVICE_NUMBER"`},
		},
		MaxRunTime: 30,
	...
	logText := LogText(t)
	if !strings.Contains(logText, fmt.Sprintf("Device: %d", config.LoopbackAudioDeviceNumber)) {
		t.Fatalf("Expected log to contain 'Device: %s', but it didn't\n%s", devicePaths[0], logText)
	}
